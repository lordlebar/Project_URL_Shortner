...
function init() {
    ...
    if (search) {
        ...
        fetch("../../managers/getlinksUser.php?email=" + email)
            ...
            .then((data) => {
                // code pour afficher les données ici
                console.log(data);
                createUrlUserTable(data);
            });
    }
}

function createUrlUserTable(urls) {
    $(`#bodyOfUrlsUserArray`).empty();
    if (urls.length == 0) {
        ...
        tr.appendChild(td);
        bodyOfUrlsUserArray.appendChild(tr);
        return;
    }
...
function updateUrl(row, tr) {
    ...
    $(`.modal-data-email_user`).val(row.email);
}

function deleteUrl(row, tr) {
    $(`#deleteUrlUserModal`).modal("show");
    $(`.modal-data-short_url`).text(row.short_url);
    $(`.modal-data-short_url`).val(row.short_url);
    $(`.modal-data-email`).text(row.email);
