<!-- Update Modal -->
<div id="updateUrlUserModal" class="modal fade" tabindex="-1" role="dialog" aria-hidden="true" data-keyboard="false" data-backdrop="static">
        ...
        <div class="modal-content">
            <form action="linkUser.php?email=<?php echo $_GET['email']; ?>" method="POST">
                ...
                <div class="modal-body">
                    <div class="mb-2">
                        <h6>for email <span class="modal-data-email text-danger"></h6>
                        <input type="hidden" name="update_email_user" class="modal-data-email_user">
                    </div>
                    <div class="mb-3">
...
if (isset($_POST["update_short_url"]) && isset($_POST["update_hidden_short_url"]) && isset($_POST["update_long_url"]) && isset($_POST["update_hidden_long_url"]) && isset($_POST["update_email_user"])) {
    ...
    // on vérifie que les champs ne sont pas vides
        ...
        if (strlen($update_short_url) < 3) {
            ...
            update_url($update_short_url, $update_hidden_short_url, $update_long_url, $email);
            echo "<script>$('#update_url_success').show('medium'); setTimeout(function(){ $('#update_url_success').hide('medium'); }, 5000);</script>";
        }
    } else {
        echo "<script>$('#fill_all_fields').show('medium'); setTimeout(function(){ $('#fill_all_fields').hide('medium'); }, 5000);</script>";
    }
}
?>
<!--/ Update modal -->