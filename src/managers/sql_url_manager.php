...
function find_url_by_long_url($long_url, $email)
{
	...
	if ($email)
		$query = "SELECT email, short_url, nb_click, long_url FROM urls WHERE long_url = '$long_url' AND email = '$email';";
	else
		$query = "SELECT email, short_url, nb_click, long_url FROM urls WHERE long_url = '$long_url' AND email IS NULL;";
...
function is_short_url_exists($short_url)
{
	$query = "SELECT 1 FROM urls WHERE short_url = '$short_url';";
	$res = query($query);
	if ($res->num_rows != 0)
		return 1;
	return 0;
...
{
	$query = "SELECT * FROM urls WHERE email = '$email';";
	return query($query);
}
...
function update_url($modify_short_url, $previous_short_url, $modify_long_url, $email)
{
	$query = "UPDATE urls SET short_url = '$modify_short_url', long_url = '$modify_long_url' WHERE short_url = '$previous_short_url' AND email = '$email';";
	query($query);
}
